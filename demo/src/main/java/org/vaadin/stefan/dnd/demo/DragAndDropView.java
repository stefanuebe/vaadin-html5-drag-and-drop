...
public class DragAndDropView extends VerticalLayout {
    ...
    public DragAndDropView() {
        ...
        dropTargetExtension.addDropListener(event -> {
            event.getDragSource().ifPresent(e -> event.getComponent().add(e.getComponent()));
            showStickyEventNotification("Event drop outer: " + event.getDragSource() + " dropped into " + event.getComponent());
        });
        ...
        innerDropTargetExtension.addDropListener(event -> {
            event.getDragSource().ifPresent(e -> event.getComponent().add(e.getComponent()));
            showStickyEventNotification("Event drop inner: " + event.getDragSource() + " dropped into " + event.getComponent());
        });
        ...
        innerInnerDropTargetExtension.addDropListener(event -> {
            event.getDragSource().ifPresent(e -> event.getComponent().add(e.getComponent()));
            showStickyEventNotification("Event drop inner inner: " + event.getDragSource() + " dropped into " + event.getComponent());
        });
    ...
    private void showStickyEventNotification(String s) {
        if (showEvents.getValue()) {
            Notification.show(s, -1, POSITION);
        }
    }
